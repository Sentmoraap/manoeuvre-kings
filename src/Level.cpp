...
float Level::dot(sf::Vector2f a, sf::Vector2f b)
{
    return a.x * b.x + a.y * b.y;
}

bool Level::overlap(float aMin, float aMax, float bMin, float bMax)
{
...
bool Level::checkSat(sf::FloatRect rect, sf::Vector2f center, float angle) const
{
    ...
    for(const Wall &wall : walls)
    {
        ...
        float base = dot(relOrigin, axis0), d20 = dot(axis2, axis0), d30 = dot(axis3, axis0);
        float diff = fabs(d20) * a2Diff + fabs(d30) * a3Diff;
        if(!overlap(a0Min, a0Max, base - diff, base + diff)) continue;
        base = dot(relOrigin, axis1);
        float d21 = dot(axis2, axis1), d31 = dot(axis3, axis1);
        diff = fabs(d21) * a2Diff + fabs(d31) * a3Diff;
        if(!overlap(a1Min, a1Max, base - diff, base + diff)) continue;
        relOrigin = -relOrigin;
        base = dot(relOrigin, axis2);
        ...
        if(!overlap(-a2Diff, a2Diff, min, max)) continue;
        base = dot(relOrigin, axis3);
        min = base, max = base;
        if(d30 > 0)
        {
            min += a0Min * d30;
        ...
        if(overlap(-a3Diff, a3Diff, min, max)) return true;
    }
    return false;
}
